...
def build_derive_context(parent_prd_path: Path, parent_arch_path: Path, target_module: str) -> dict:
    ...
    # 4. Detect orphan requirements — requirements that don't match ANY module
    all_modules_keywords = list(arch_result["available_modules"])
    orphan_requirements = []
    for req in all_requirements:
        req_text = req.get("text", "")
        matched_any = False
        for kw in all_modules_keywords:
            if kw in req_text:
                matched_any = True
                break
        if not matched_any:
    ...
    # 5. Extract interfaces and dependencies
